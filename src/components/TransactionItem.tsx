...
import { ArrowDownLeft, ArrowUpRight, Ellipsis } from "lucide-react-native";
...
export function TransactionItem({ transaction, onPress }: { transaction: Transaction; onPress: () => void }) {
  ...
  return (
    <Pressable accessibilityRole="button" onPress={onPress} style={({ pressed }) => [styles.row, { backgroundColor: pressed ? colors.surfaceMuted : colors.surface }]}>
...
const styles = StyleSheet.create({
  row: { minHeight: 68, borderBottomWidth: 1, flexDirection: "row", alignItems: "center", gap: 12, paddingVertical: 12 },
