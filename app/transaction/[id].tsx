...
export default function TransactionDetailScreen() {
  ...
  useEffect(() => { void getTransaction(db, id).then((item) => { if (!item) return; setTransaction(item); setType(item.type); setAmountCents(item.amountCents); setCategoryId(item.categoryId); setDescription(item.description ?? ""); setOccurredAt(item.occurredAt); }); }, [db, id]);
  ...
  const save = useCallback(async () => {
    const draft = { type, amountCents, categoryId: categoryId ?? "", description, occurredAt };
  ...
  }, [categoryId, db, description, id, occurredAt, type, amountCents]);

  function confirmDelete() { Alert.alert("Excluir lançamento?", "Essa ação não pode ser desfeita.", [{ text: "Cancelar", style: "cancel" }, { text: "Excluir", style: "destructive", onPress: () => { void deleteTransaction(db, id).then(() => router.back()); } }]); }
